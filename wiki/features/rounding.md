...
A estratégia de arredondamento é definida na criação da instância, via `CalcAUY.create({ roundStrategy: '...' })`, e aplicada nos métodos de saída como `toStringNumber()`. O `commit()` não recebe parâmetros.
